...
function Result({ result, lineup, formationId, onReset, onReplay }: { result: FootballSeasonResult; lineup: LineupPick[]; formationId: FootballFormationId; onReset: () => void; onReplay: () => void }) {
  ...
  const userCupMatches = result.cup.filter((match) => match.homeId === result.clubId || match.awayId === result.clubId);
  const fullJourney = [...userMatches, ...userCupMatches];
  const leagueChampion = getFootballClub(result.leagueChampionId);
  const cupChampion = getFootballClub(result.cupChampionId);
  const tabs: Array<{ id: ResultTab; label: string }> = [{ id: "overview", label: "Season HQ" }, { id: "fixtures", label: "Full journey" }, { id: "table", label: "League table" }, { id: "cup", label: "Cup run" }, { id: "lineup", label: "Starting XI" }];
  return <section className={styles.result} style={{ "--club-primary": club.primary, "--club-secondary": club.secondary } as React.CSSProperties}>
    ...
    {tab === "overview" && <div className={styles.overview}><div className={styles.summary}><article><span>League position</span><strong>#{result.leaguePosition}</strong><small>{result.userPoints} points</small></article><article><span>Goals scored</span><strong>{result.userGoalsFor}</strong><small>{(result.userGoalsFor / 38).toFixed(2)} per match</small></article><article><span>Goal difference</span><strong>{result.userGoalsFor - result.userGoalsAgainst > 0 ? "+" : ""}{result.userGoalsFor - result.userGoalsAgainst}</strong><small>{result.userGoalsAgainst} conceded</small></article><article><span>Top scorer</span><strong>{result.starGoals}</strong><small>{result.starPlayer}</small></article></div><div className={styles.overviewGrid}><article className={styles.formCard}><div className={styles.cardHead}><div><span>Complete campaign</span><h2>Match-by-match form</h2></div><strong>{fullJourney.length} total matches</strong></div><div className={styles.outcomeLegend} aria-label="Result colours"><span><i className={styles.win} />Win</span><span><i className={styles.draw} />Draw</span><span><i className={styles.loss} />Loss</span></div><div className={styles.formGrid}>{fullJourney.map((match, index) => { const userGoals = match.homeId === result.clubId ? match.homeGoals : match.awayGoals; const opponentGoals = match.homeId === result.clubId ? match.awayGoals : match.homeGoals; const outcome = userGoals > opponentGoals ? "Win" : userGoals < opponentGoals ? "Loss" : "Draw"; return <i key={match.id} className={outcome === "Win" ? styles.formWin : outcome === "Loss" ? styles.formLoss : styles.formDraw} title={`${match.label}: ${outcome}`}>{index + 1}</i>; })}</div><div className={styles.recentMatches}>{fullJourney.slice(-5).reverse().map((match) => <MatchRow key={match.id} match={match} userClubId={result.clubId} />)}</div></article><aside className={styles.dna}><span>Team DNA</span><h2>{result.metrics.overall} overall</h2><Metrics metrics={result.metrics} /><div><span>Replayable seed</span><strong>{result.seed}</strong></div></aside></div><div className={styles.trophyStrip}><article><span>League</span><strong>{leagueChampion.name}</strong><small>{result.leaguePosition === 1 ? "Your club lifted the title" : "Finished champions"}</small></article><article><span>Knockout cup</span><strong>{cupChampion.name}</strong><small>{result.cupFinish}</small></article></div><div className={styles.actions}><button type="button" className={styles.secondary} onClick={onReplay}>Replay same XI</button><button type="button" className={styles.primary} onClick={onReset}>Draft another dynasty <span>Start over →</span></button></div></div>}
    {tab === "fixtures" && <div className={styles.panel}><div className={styles.cardHead}><div><span>League through the final whistle</span><h2>Your complete campaign journey</h2></div><strong>{fullJourney.length} matches played</strong></div><div className={styles.outcomeLegend} aria-label="Result colours"><span><i className={styles.win} />Win</span><span><i className={styles.draw} />Draw</span><span><i className={styles.loss} />Loss</span></div><section className={styles.journeyStage}><h3>League season · Matchweeks 1–38</h3><div className={styles.matchList}>{userMatches.map((match) => <MatchRow key={match.id} match={match} userClubId={result.clubId} />)}</div></section><section className={styles.journeyStage}><h3>Your knockout cup run · {userCupMatches.length} match{userCupMatches.length === 1 ? "" : "es"}</h3><div className={styles.matchList}>{userCupMatches.map((match) => <MatchRow key={match.id} match={match} userClubId={result.clubId} />)}</div></section></div>}
