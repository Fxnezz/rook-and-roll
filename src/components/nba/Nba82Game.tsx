...
function Result({ result, lineup, onReset, onReplay }: { result: NbaSeasonResult; lineup: LineupPick[]; onReset: () => void; onReplay: () => void }) {
  ...
  const team = getNbaTeam(result.teamId); const userGames = result.regularSeason.filter((game) => game.homeId === result.teamId || game.awayId === result.teamId); const userPostseason = [...result.playIn, ...result.series.flatMap((series) => series.games)].filter((game) => game.homeId === result.teamId || game.awayId === result.teamId); const fullJourney = [...userGames, ...userPostseason]; const perfect = result.userWins === 82;
  const tabs: Array<{ id: ResultTab; label: string }> = [{ id: "overview", label: "Season HQ" }, { id: "schedule", label: "Full journey" }, { id: "standings", label: "Standings" }, { id: "playoffs", label: "Playoffs" }, { id: "lineup", label: "Starting five" }];
  return <section className={styles.result} style={{ "--team-primary": team.primary, "--team-secondary": team.secondary } as React.CSSProperties}>
    ...
    {tab === "overview" && <div className={styles.overview}><div className={styles.summary}><article><span>{team.conference} seed</span><strong>#{result.conferenceSeed}</strong><small>{result.userWins / 82 >= .5 ? `${(result.userWins / 82 * 100).toFixed(1)} win %` : "Rebuild season"}</small></article><article><span>Points per game</span><strong>{(result.pointsFor / 82).toFixed(1)}</strong><small>{result.pointsFor.toLocaleString()} total</small></article><article><span>Allowed per game</span><strong>{(result.pointsAgainst / 82).toFixed(1)}</strong><small>{result.pointsAgainst.toLocaleString()} total</small></article><article><span>Best streak</span><strong>{result.streak}</strong><small>straight wins</small></article></div><div className={styles.overviewGrid}><article className={styles.formCard}><div className={styles.cardHead}><div><span>Complete campaign</span><h2>Game-by-game form</h2></div><strong>{fullJourney.length} total games</strong></div><div className={styles.outcomeLegend} aria-label="Result colours"><span><i className={styles.win} />Win</span><span><i className={styles.loss} />Loss</span></div><div className={styles.formGrid}>{fullJourney.map((game, index) => <i key={game.id} className={game.winnerId === result.teamId ? styles.formWin : styles.formLoss} title={`${game.label}: ${game.winnerId === result.teamId ? "Win" : "Loss"}`}>{index + 1}</i>)}</div><div className={styles.recentGames}>{fullJourney.slice(-5).reverse().map((game) => <GameRow key={game.id} game={game} userTeamId={result.teamId} />)}</div></article><aside className={styles.dna}><span>Team DNA</span><h2>{result.metrics.overall} overall</h2><Metrics metrics={result.metrics} /><div><span>Replayable seed</span><strong>{result.seed}</strong></div></aside></div><div className={styles.actions}><button type="button" className={styles.secondary} onClick={onReplay}>Replay same five</button><button type="button" className={styles.primary} onClick={onReset}>Draft another dynasty <span>Start over →</span></button></div></div>}
    {tab === "schedule" && <div className={styles.panel}><div className={styles.cardHead}><div><span>Regular season through elimination</span><h2>Your complete campaign journey</h2></div><strong>{fullJourney.length} games played</strong></div><div className={styles.outcomeLegend} aria-label="Result colours"><span><i className={styles.win} />Win</span><span><i className={styles.loss} />Loss</span></div><section className={styles.journeyStage}><h3>Regular season · Games 1–82</h3><div className={styles.gameList}>{userGames.map((game) => <GameRow key={game.id} game={game} userTeamId={result.teamId} />)}</div></section>{userPostseason.length > 0 ? <section className={styles.journeyStage}><h3>Your postseason · {userPostseason.length} games</h3><div className={styles.gameList}>{userPostseason.map((game) => <GameRow key={game.id} game={game} userTeamId={result.teamId} />)}</div></section> : <div className={styles.journeyEmpty}><strong>Regular season complete</strong><span>Your team missed the postseason, so the journey ends after Game 82.</span></div>}</div>}
